...
module I2C_CONTROLLER #(
  ...
  // FIXME: The next two are not connected
...
// FIXME: In general, make sure there is no way for an X to get in here.
// (Initialize everything, make sure all case statements have defaults, etc.)
...
localparam CLK_DIV_CNT_START = { {(CLK_CNT_SZ-1){1'b0}}, 1'b1 }; // 1 in the proper bit width
...
logic [1:0] step = 2'b0;
logic [3:0] byte_step = 4'b0; // 0-7 = get/send a bit (MSB first); 8 = send/get ACK
logic [2:0] byte_idx = 3'h7;  // Counts down from 7 to 0 for sending/getting a byte MSB first

// This next one is a hack for now
          ...
          // (If multi-controller, we would want to check the inputs...)
			...
			S_DATA2:   ////////////////////////////////////////////////////
				begin
					...
					// For now we're not implementing read
					// TODO: Implement read
					is_write = 1; // Intentional combinatoric
					...
					if (byte_step == 4'd8) begin
						...
						if (is_write) case (step)
							...
							0: begin
									...
									scl_e <= 1; scl_o <= 0;
									sda_e <= 0; // This should tristate the output
									got_ack <= 0;
								end
							1: begin
            ...
            // 1. We're at the very end of a byte, so advance state
						// TODO: Check if we got_ack first if we're writing;
			...
			S_STOP: begin /////////////////////////////////////////////////
				...
				// TODO: Store the abort/success flags
        // Abort = any step did not get ACK
        // Success = ~Abort
