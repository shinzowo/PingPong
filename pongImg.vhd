...
architecture imagegen of pongImg is
    ...
    signal bar_top : integer range 2047 downto 0 := 204;
    ...
    signal lbar_top : integer range 2047 downto 0 := 204;
    signal lbar_bottom : integer range 2047 downto 0 := 275;

    signal rbar_pos_left : integer :=  594;
    signal rbar_pos_right : integer := 609;
    signal rbar_top : integer range 2047 downto 0 := 204;
    signal rbar_bottom : integer range 2047 downto 0 := 275;

    signal init_ball_pos_left : integer := 316;
    signal init_ball_pos_right : integer := 323;
    signal init_ball_top : integer := 19;
    signal init_ball_bottom : integer := 26;
    signal ball_pos_left : integer range 2047 downto 0 := 316;
    signal ball_pos_right : integer range 2047 downto 0 := 323;
    signal ball_pos_left_next : integer range 2047 downto 0;
    signal ball_top_next : integer range 2047 downto 0;
    signal ball_top : integer range 2047 downto 0 := 19;
    signal ball_bottom : integer range 2047 downto 0 := 26;
...
begin
    ...
    -- Динамический пересчет размеров поля
    process(mode)
    begin
        case mode is
            when "01" => active_width <= 800; active_height <= 600;
            when "10" => active_width <= 1024; active_height <= 768;
            when others => active_width <= 640; active_height <= 480;
        end case;
    end process;
    ...
    rbar_pos_right <= active_width - 31;

    init_ball_pos_left <= center_x - 4;
    init_ball_pos_right <= center_x + 3;
    bar_top <= (active_height / 2) - 36;
...
process(framerate, img_reset, img_restart, game_finished,
                   lbar_top, lbar_bottom, lbutton_p1, rbutton_p1, active_height)
begin
    if (img_reset = '1') then
        lbar_top <= bar_top;
        ...
        if game_finished = '0' then
            ...
            elsif (lbutton_p1 = '0' and rbutton_p1 = '1' and lbar_bottom <= active_height - 5) then
                lbar_top <= lbar_top + bar_vel;
            else  
                lbar_top <= lbar_top;
            end if;
...
process(framerate, img_reset, img_restart, game_finished,
                   rbar_top, rbar_bottom, lbutton_p2, rbutton_p2, active_height)
begin
    if (img_reset = '1') then
        rbar_top <= bar_top;
    elsif (framerate'event and framerate = '1') then
        if game_finished = '0' then
            ...
            elsif (lbutton_p2 = '0' and rbutton_p2 = '1' and rbar_bottom <= active_height - 5) then
                rbar_top <= rbar_top + bar_vel;
            else 
                rbar_top <= rbar_top;
            end if;
...
process(framerate, img_reset, img_restart, rsdelay, ball_team, game_finished,
                   ...
                   ball_pos_left_next, ball_top_next, active_width, active_height)
begin
    if (img_reset = '1') then
        ...
        elsif (ball_bottom >= active_height - 4 ) then
        ...
        elsif (ball_pos_right >= active_width - 4) then
